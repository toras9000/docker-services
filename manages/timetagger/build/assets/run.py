...
async def get_webtoken_usernamepassword(request, auth_info):
    ...
    passwd = auth_info.get("password", "").strip()

    from ldap3 import Server, Connection, Tls, ALL

    #
    import os
    ldap_host = os.environ.get("TIMETAGGER_LDAP_HOST", None)
    ldap_dn = os.environ.get("TIMETAGGER_LDAP_DN", None)
    if (ldap_host is None) or (ldap_dn is None):
        return 403, {}, "No auth settings"
    ldap_user_dn = ldap_dn.format(user)

    ldap_ssl = False
    env_ssl = os.environ.get("TIMETAGGER_LDAP_SSL", None)
    if (env_ssl is not None):
        from distutils.util import strtobool
        ldap_ssl = strtobool(env_ssl)

    ldap_port = 636 if ldap_ssl else 389
    env_port = os.environ.get("TIMETAGGER_LDAP_PORT", None)
    if (env_port is not None):
        ldap_port = int(env_port)

    ldap_tls = None
    if ldap_ssl:
        env_cert = os.environ.get("TIMETAGGER_LDAP_CA_CERT", None)
        if (env_cert is not None):
            ldap_tls = Tls(ca_certs_file=env_cert)

    logger.info(f"LDAP auth: {ldap_host}:{ldap_port}, ssl={ldap_ssl}, user={user}")

    try:
        server = Server(ldap_host, port=ldap_port, use_ssl=ldap_ssl, tls=ldap_tls, get_info=ALL)
        connection = Connection(server, user=ldap_user_dn, password=passwd, version=3, authentication='SIMPLE')
        if connection.bind():
            connection.unbind()
            token = await get_webtoken_unsafe(user)
            return 200, {}, dict(token=token)
        ...
        logger.error(f"Failed to auth: {err}")
        return 401, {}, "Failed to auth"
