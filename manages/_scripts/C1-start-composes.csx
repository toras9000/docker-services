...
await "dotnet".args("script", ThisSource.RelativeFile("C0-prepare-networks.csx").FullName);
await "dotnet".args("script", ThisSource.RelativeFile("C0-prepare-volumes.csx").FullName);

var endpoints = new List<ServiceEndpoint>();
foreach (var info in ManageServices.GetContainers())
{
    WriteLine($">{info.Compose.File.Directory?.Name}");
    await "docker".args(["compose", "--file", info.Compose.File.FullName, "up", "-d", .. info.Compose.UpArgs ?? []]);
